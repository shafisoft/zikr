/**
 * Create Room Modal — multi-step group creation.
 * Step 1 "details": name, group title, first plan (zikrs, target mode,
 * period) via GroupPlanBuilder.
 * Step 2 "share": group code + copy-link actions.
 * Noor design system.
 */
...
const CreateRoomModal: React.FC<CreateRoomModalProps> = ({ isOpen, onClose, onCreated }) => {
  ...
  const handleCreate = async () => {
    ...
    if (!title.trim()) {
      setLocalError(t('createRoom.titleRequired'));
      return;
    }
    ...
    try {
      ...
      const room = await createRoom({
        title: title.trim(),
        initialPlan: groupPlanToInput(draft),
        windowType: draft.period === 'one-time' ? draft.preset : draft.period,
      });
      setCreated(room);
      setStep('share');
    } catch (err) {
