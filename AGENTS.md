...
- **Client-side RPC contract:** `src/core/services/supabaseTypes.ts` is the single TypeScript mirror of the RPC surface — every function's SQL parameter names and json return shape (`SupabaseRpc`). Both Supabase adapters route all calls through it, so signature/payload drift between the migrations and the client fails `tsc` instead of shipping a runtime crash (this class of bug — client reading `room` where the server sends `group` — reached production once). When changing a function, edit the migration first, then that file; the port contracts (`sharedRoom/contract.ts`, `zikrSync/contract.ts`) alias its shapes under backend-agnostic names.
